...
def dealdata(model,num):
    ...
    for i in range(num):
        ...
        a = np.reshape(np.array(pic), [3600])
...
def train(size,model):
    ...
    y_ = tf.placeholder(tf.float32,shape=[None,35*size])
    ...
    W_fc2 = weight_variable([1024, 35*size])
    b_fc2 = bias_variable([35*size])
    ...
    correct_prediction = tf.equal(tf.argmax(y_conv[0:35], 1), tf.argmax(y_[0:35], 1))
    ...
    for onekey in range(1, size):
        correct_prediction = tf.equal(tf.argmax(y_conv[size*onekey:size*onekey+35], 1), tf.argmax(y_[size*onekey:size*onekey+35], 1))
        accuracy += tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            train_step.run(feed_dict={x: part_data, y_: part_label, keep_prob: 0.5})
            ...
            if i%1000==0 and i!=0:
                ...
                os.mkdir('model/+'+model+'train_cnn_captcha_after'+str(i/1000)+'k')
                saver.save(sess, 'model/+'+model+'train_cnn_captcha_after'+str(i/1000)+'k/model')
...
def load(model_path,size,model):
    ...
    predict = tf.reshape(y_conv, [-1, 4, 35])
    max_idx_p = tf.argmax(predict, 2)
    max_idx_l = tf.argmax(tf.reshape(y_, [-1, 4, 35]), 2)
    ...
    with tf.Session() as sess:
        ...
        file = open('result_num_and_letter.txt','a')
        all = 0
...
if __name__ == '__main__':
    # dealdata('train1',20000)
    # dealdata('train2', 20000)
    # train(4,'train')
    load('model/train_letter_19k/model',4,'test')
