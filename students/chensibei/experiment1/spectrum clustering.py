...
def Laplacian(sigma,data):#计算拉普拉斯矩阵
    PointNumber = shape(data)[0]
    WMatrix =mat(zeros((PointNumber,PointNumber)))

    D =mat(zeros((PointNumber,PointNumber)))
    L =mat(zeros((PointNumber,PointNumber)))

    for i in range(0,PointNumber):#计算关系矩阵
        for j in  range(0,PointNumber):
            if(i==j):
                WMatrix[i,j] = 0
                continue
            else:
                WMatrix[i,j] = exp(- distEclud(data[j, :], data[i, :])  / (2 * sigma*sigma))#get related matrix
                continue

    for i in range(PointNumber):#计算度矩阵
		 for j in range(PointNumber):
			  D[i,i] += WMatrix[i,j]

    for i in range(PointNumber):#计算拉普拉斯矩阵
		for j in range(PointNumber):
			L[i,j] = D[i,j] - WMatrix[i,j]

    return L

	
def TOPK(sigma,k,data):
    LA = Laplacian(sigma,data)
    
   
    a,b=np.linalg.eig(LA)
    evals=a
    evecs=b

    indices = np.argsort(evals)[:k]  
    return normalize(evecs[:, indices])
    # return evecs[:, indices]




def InitNewPoint(sigma,k,data):
    PointNumber = shape(data)[0]
    newpoint =mat(zeros((PointNumber,k)))
    top_evecs = mat(zeros((PointNumber,k)))
    top_evecs = TOPK(sigma,k,data)
    for i in range(PointNumber):
        for j in range(k):
            newpoint[i,j] = top_evecs[i,j]
    return  newpoint
...
def RandK(data, k):#随机给出k个中心点
    ...
    for j in range(n):
        ...
        randk[:,j] = Min +(Range)* random.rand(k,1)
...
newpoint = InitNewPoint(1,10,data)
RANDK, TAP= kMeans(newpoint,kk)
